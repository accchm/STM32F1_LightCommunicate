...
{      			    
    ...
    for(t=0;t<csize;t++)
    {   
		if(size==12)temp=OLED_ascii_1206[chr][t]; 	 	//调用1206字体
		else if(size==16)temp=OLED_ascii_1608[chr][t];	//调用1608字体
		else if(size==24)temp=OLED_ascii_2412[chr][t];	//调用2412字体
		else return;								//没有的字库
        for(t1=0;t1<8;t1++)
		{
...
void OLED_Show_Float(u8 x,u8 y,float num, u8 float_len,u8 size)
{
	float n;
	n = num;
	OLED_ShowNum(x,y,n,2,size);
	OLED_ShowChar(x+size,y,'.',size,1);
	OLED_ShowNum(x+size+4,y,(int)(n*oled_pow(10,float_len))%oled_pow(10,float_len),float_len,size);
}
